...
public class Population implements Cloneable{
	...
	public String[] fillPop(String[] pop){
		for(int x = 0; x < getPopulationSize(); x++){
			...
			for(int y = 0; y < getSizeChrmosome(); y++){
				...
				if(y == 0){
					pop[x] = numberCars.get(carNumber).toString();
					carNumber++;
					fChangePosition = iChangePosition;
				} else { 
					if(y < fChangePosition){ //Insere os pontos de entrega (Clientes)
						pop[x] = pop[x] + numberClient.get(clientNumber).toString();
						clientNumber++;
					} else { //Insere os próximos carros
						pop[x] = pop[x] + numberCars.get(carNumber).toString();
						carNumber++;
						fChangePosition = fChangePosition + iChangePosition;
					}
				}
			}
		}
	 ...
	 * @return
	 */
	public String[] startPop(int popSize, int qttClients, int qttCars){
